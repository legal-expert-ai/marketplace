...
The approved source must be one DOC/DOCX and the signed source must be one PDF in the same creator-owned ready bundle. Preview v1 accepts their exact file IDs plus the bundle ID; upload handles, file bytes, base64, arbitrary URLs and local paths are forbidden.
